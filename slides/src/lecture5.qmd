...
subtitle: Lecture 5 - Single Variable Calculus & Optimization
...
- Functions of one variable take a single input and produce a single output. This is usually denoted as:
...
- The concept of slope is crucial for understanding rates of change, as it is the ratio of the change in $y$ to the change in $x$.
...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=0.7]
        ...
        % Legend (fixed position, so labels never fall outside the axis)
        \node[right, align=left] at (axis cs: -0.9, 9.3) {\footnotesize\textcolor{blue}{$f(x) = x^2$}};
        \node[right, align=left] at (axis cs: -0.9, 8.2) {\footnotesize\textcolor{red}{Tangent at $x = 1$}};
        \node[right, align=left] at (axis cs: -0.9, 7.1) {\footnotesize\textcolor{green!60!black}{Tangent at $x = 3$}};
...
- A limit is an operator that gives the value a function **approaches** as the input approaches a certain value. It helps to think of it as the value the function gets closer and closer to.

- There are many rules and properties of limits that are used to calculate them. We will not cover them in this lecture, but they are crucial for a deeper understanding of calculus.
...
- The concept of **margin** is crucial in economics and business, denoting the **additional** or **incremental** change in a quantity given a small change in another quantity.
...
Suppose a car dealership's profit function is

$$\Pi(q) = 200q - q^2$$

where $q$ is the number of cars sold.

# Example: marginal profit

What is the marginal profit? This means we need to calculate the derivative of profit with respect to the number of cars sold:

$$\dfrac{d\Pi}{dq} = 200 - 2q$$

- This derivative is the **marginal profit** function.

- It tells us approximately how much profit changes when the dealership sells one more car.

- For example, at $q = 50$,

$$MP(50) = 200 - 2(50) = 100$$

- So near 50 cars sold, one additional car raises profit by about \$100.
...
- Critical points are important because they can be local maxima, local minima, or neither.
    ...
    - Some critical points are neither a maximum nor a minimum.
...
- There are several ways to do this:
    ...
    - Look at the graph of the function around the critical points.
...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=0.7]
        ...
        \addplot[
            ...
            mark=*,
            color=black
        ] coordinates {(1 + sqrt(3)/3, -0.3849) (1 - sqrt(3)/3, 0.3849)};
...
- From the graph, we can see that the function has a local minimum at $x = 1 + \dfrac{\sqrt{3}}{3}$ and a local maximum at $x = 1 - \dfrac{\sqrt{3}}{3}$.
...
- We are interested in maximization when functions exhibit at least a local maximum, and in minimization when they exhibit at least a local minimum.

- Whether a function has maxima or minima is a question of interest in many fields, and it depends on the shape of the function and the behavior of its derivatives.

- We often talk about concavity and convexity of functions to determine whether they have maxima or minima. 

- As a rough rule, if a function is concave up, it tends to have a minimum, and if it is concave down, it tends to have a maximum. 
...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=0.7]
        ...
        % Legend (fixed position, so labels never fall outside the axis)
        \node[right, align=left] at (axis cs: -0.9, 1.8) {\footnotesize\textcolor{blue}{$f(x) = x^2$}};
        \node[right, align=left] at (axis cs: -0.9, 1.4) {\footnotesize\textcolor{red}{$f(x) = -x^2$}};
...
- A real world example of optimization is maximizing or minimizing a function that represents a quantity of interest.
    - For example, a government may want to maximize public welfare given a certain budget.
    - A firm may want to maximize profit or minimize cost.
    - An investor may want to choose the scale of an investment to maximize returns net of cost.

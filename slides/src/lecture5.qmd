...
- A quantitative model uses quantitative information (numbers, equations, etc.) to represent a real-world system.
...
- Sometimes whether something in the model changes might not be obvious
        - Depends on the context of the model.
...
- If we relax the assumption of perfect competition, the conclusions of the model typically change.
...
- If we estimate an elasticity of 0.20, we multiply it by the percentage change we expect in $X$ to approximate the percentage change in $Y$.
    - Elasticities do not always behave perfectly linearly, but they are often a useful approximation.
...
- In the social sciences, where we rely on observational rather than experimental studies, I argue that these are not the best terms to use.
    - Independent variables, while often exogenous, are not always under our control or may be indirectly affected by other variables.
...
- In an estimation context, we hear the word function a lot. For example, the demand function, the supply function, the production function, and the utility function.
...
- For example:
    ...
    - A sales function relating advertising to revenue
...
- **Functional form**: The way that a function is written, often referring to its polynomial degree or general type.
...
- Structural estimation means taking the model very seriously, and estimating the parameters of the model directly.
    ...
    - It often requires stronger mathematical assumptions and more advanced statistical techniques.
...
- Reduced form estimation means estimating the parameters of the model indirectly, by estimating the relationship between the variables.
    ...
    - It usually requires less mathematics and simpler statistical techniques.
